...
- **Engineering.** Every primitive that lands also has to ship a real-time WGSL implementation that runs at 60+ fps on a consumer card. The math has to survive the trip from textbook to GPU shader, and the rigid-body physics has to settle into stable stacks the way you'd expect.
...
Hardware-agnostic via wgpu. Vulkan, DX12, Metal, WebGPU. No CUDA lock-in. If ML features eventually show up, they go through Burn's wgpu backend first.
