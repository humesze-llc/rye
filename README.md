...
- **Geometry as a type parameter.** Seven `Space` implementations behind one trait (exp, log, distance, parallel transport, isometries): Euclidean R²/R³/R⁴, hyperbolic H³, spherical S³ in two charts, and a variable-curvature E³-to-H³ blend. `cargo test -p loam-math --test space_conformance` runs one invariant list against every one of them; `cargo test -p loam-shader --lib -- --include-ignored wgsl_matches_the_rust_space` pins each closed-form WGSL prelude against its Rust twin on a real adapter. Downstream support is per-space and partial: `WgslSpace` (the raymarcher) covers E³, R⁴, H³, S³, and the blend; `RasterizableSpace` covers E³, R⁴, and the embedded S³, so E³ and R⁴ are the two where both paths share one implementation and H³ has no rasterizer path. The shipped demos run the Euclidean spaces; the curved-space paths are exercised by those tests, not yet by a runnable demo.
