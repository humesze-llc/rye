//! `Space::exp` and `Space::log` are quotient operations; `exp` returns the
//! canonical representative. `IsometryGroup::iso_apply` is a covering-space
//! operation and deliberately does not re-canonicalize: canonicalizing would
//! collapse Γ to the identity and destroy [`QuotientSpace::wrap_to_domain`]'s
//! postcondition, the one statement that lets a caller ask which deck element
//! a crossing used.
...
pub trait QuotientSpace: IsometryGroup {
    // Order is part of the contract: products of generators must be
