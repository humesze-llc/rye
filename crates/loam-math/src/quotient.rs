//! `exp` and `log` return canonical representatives; `iso_apply` acts on the
//! cover and does not re-canonicalize, so [`QuotientSpace::wrap_to_domain`]
//! can report which deck element a crossing used.
...
pub trait QuotientSpace: IsometryGroup {
    ...
    /// Exactly the fixpoints of [`QuotientSpace::wrap_to_domain`]; one per orbit.
    fn in_fundamental_domain(&self, p: Self::Point) -> bool;

    /// The canonical lift `q` of `p` and the deck element `g` with
    /// `iso_apply(g, p) == q`.
    fn wrap_to_domain(&self, p: Self::Point) -> (Self::Point, Self::Iso);
}

/// Deck group Z³ on the half-open box `[-cᵢ/2, cᵢ/2)`; a rectangular lattice
/// makes the nearest-point reduction exact (Conway & Sloane 1988, *Sphere
/// Packings, Lattices and Groups*, ch. 4 §1).
...
impl FlatTorus3 {
    ...
    // The residual correction closes the case where `pᵢ/cᵢ` rounds onto the tie.
...
impl Space for FlatTorus3 {
    ...
    // Minimal representative; inverts `exp` only inside the injectivity radius.
    ...
    // Chart arithmetic is not lattice-periodic, so it is not the geometry.
...
impl QuotientSpace for FlatTorus3 {
    ...
    fn wrap_to_domain(&self, p: Vec3) -> (Vec3, Iso3) {
        ...
        // `p + translation` reproduces `iso_apply` bit for bit.
...
// GPU parity is one ulp; on a face the shader may return the lift one cell over.
// `{:?}` always prints a decimal point or exponent, which a WGSL f32 literal needs.
...
// The quotient is compact, so the march kernel's boundary escape must never fire.
...
/// Quotient of S³ ⊂ C² by `g·(z₁, z₂) = (e^(2πi/p)·z₁, e^(2πi·q/p)·z₂)`, with
/// `z₁ = x + iy`, `z₂ = z + iw` (Rolfsen, *Knots and Links*, 1976, §9.B;
/// Hatcher, *Algebraic Topology*, 2002, Example 2.43). The fundamental domain
/// is the wedge `arg(z₁) ∈ [-π/p, π/p)`.
...
impl LensSpace {
    ...
    // Shared by membership and wrap, so membership is exactly the wrap's fixpoints.
    ...
    // Ties go to the smallest non-negative power.
    ...
    /// Not [`WgslSpace`]: that ABI is `vec3<f32>`, and this quotient marches in
    /// the ambient `vec4<f32>`, on the deck table [`LensSpace::deck`] computes.
...
// Chord and arc share the argmin over the deck group, so the asin runs once.
...
impl Space for LensSpace {
    ...
    // Minimal representative; inverts `exp` as a statement about orbits, not lifts.
    ...
    fn parallel_transport(&self, from: Vec4, to: Vec4, v: Vec4) -> Vec4 {
        ...
        // `carried` is tangent at `g^power·to`; the inverse power returns it to `to`.
...
impl IsometryGroup for LensSpace {
    ...
    // Descends to the quotient only for an `Iso4` normalising the deck group;
    // not enforced.
...
impl QuotientSpace for LensSpace {
    ...
    fn wrap_to_domain(&self, p: Vec4) -> (Vec4, Iso4) {
        ...
        // One correction closes the case where the rotation rounds past the wall.
...
mod tests {
    ...
    // Exact face coordinates, an ulp either side, and lattice translates of each.
    ...
    fn stepped_ray_continuation_matches_a_single_geodesic_exp() {
        ...
        for _ in 0..5_000 {
            ...
            // 64 wraps, each at most a cell-scaled rounding.
    ...
    fn exp_and_log_invert_exactly_within_the_injectivity_radius() {
        ...
        // Two units along y in a cell of three wraps to one unit back.
...
mod lens_tests {
    ...
    // Includes `q² ≡ 1 (mod p)` at (8, 3), where the isometry group is larger.
    ...
    // Independent of `deck_angles`' modular reduction.
    ...
    // Four decades above the rounding of the wrap's rotation and of `atan2`.
    const WALL_MARGIN: f32 = 1e-4;

    // A margin either side of every wall, off both degenerate circles.
    ...
    fn crossing_a_face_re_enters_through_the_opposite_face() {
        ...
        for &(p, q) in LENSES {
            ...
            // As an action: at p = 2 the inverse matrix differs by `sin(−π)` vs `sin(π)`.
    ...
    fn exp_and_log_invert_within_the_injectivity_radius() {
        ...
        for &(p, q) in LENSES {
            ...
            // A full deck displacement is a closed geodesic: the short way home is zero.
            let at = Vec4::X;
